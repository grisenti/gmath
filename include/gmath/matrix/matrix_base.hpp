...
&&ModifiableMatrix1D<typename M::ModifiableEquivalent> &&
        std::same_as<typename M::ComponentType,
            typename M::ModifiableEquivalent::
                ComponentType> && (M::SIZE == M::ModifiableEquivalent::SIZE)
    && std::is_base_of_v<Matrix1DTag, typename M::TypeClass>;
...
concept ModifiableRowMatrix
    = ModifiableMatrix1D<M> && std::is_base_of_v<RowMatrixTag,
        typename M::TypeClass>;

template <typename M>
concept ModifiableColumnMatrix
    = ModifiableMatrix1D<M> && std::is_base_of_v<ColumnMatrixTag,
        typename M::TypeClass>;

template <typename M>
concept ConstRowMatrix = ConstMatrix1D<M> && std::is_base_of_v<RowMatrixTag,
    typename M::TypeClass>;

template <typename M>
concept ConstColumnMatrix
    = ConstMatrix1D<M> && std::is_base_of_v<ColumnMatrixTag,
        typename M::TypeClass>;
